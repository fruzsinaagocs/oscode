...
int main(){
    ...
    std::string output = "vdp_PI_control.txt"; 
    ...
    */
    ...
    /** Solve the ODE */    
    Solution solution(sys, x0, dx0, ti, tf, 3, 1e-6, 1e-6, 1e-4);
    solution.solve();
    ...
    std::list<Eigen::Matrix<double, 1, 3>> hstats = solution.allhs;
    ...
    auto it_hs = hstats.begin();
    for(int i=0; i<=steps; i++){
        f << *it_t << ", " << std::real(*it_x) << ", " << std::imag(*it_x) <<
            ", " <<  *it_ty << ", " << (*it_hs)[0] << ", " << (*it_hs)[1] << ", "
            << (*it_hs)[2] << std::endl;
        ++it_t;
        ++it_x;
        ++it_ty;
