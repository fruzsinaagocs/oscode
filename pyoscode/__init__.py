...
import _pyoscode
#except ImportError:
#    print('pyoscode: could not find _pyoscode.so')
#    print('           Go back to your pyoscode library and run: ')
#    print('')
...
rtol=1e-4, atol=0.0, h=1.0, full_output=False):
    ...
    """Solve a differential equation of the form x(t)'' + 2*g(t)*x(t)' + w(t)^2*x(t)
    = 0 with the RKWKB method.
    ...
    gs : `numpy.ndarray` [`complex`] or `list` [`complex`]
        An array-like object of real or co
        mplex numbers representing the values of the friction term g at the points
        given in ts.
